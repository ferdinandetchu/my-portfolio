...
export function TestimonialCard({ testimonial }: TestimonialCardProps) {
  const isLongQuote = testimonial.quote.length > 100;
  const shortQuote = isLongQuote 
    ? `${testimonial.quote.substring(0, 100)}...` 
    : testimonial.quote;

  return (
    <div className={cn("h-[320px] w-full [perspective:1200px]", isLongQuote && "group")}>
      <div 
        className={cn(
            "relative h-full w-full transition-all duration-700 [transform-style:preserve-3d]", 
            isLongQuote && "group-hover:[transform:rotateY(180deg)]"
        )}
      >
        {/* Front Face */}
